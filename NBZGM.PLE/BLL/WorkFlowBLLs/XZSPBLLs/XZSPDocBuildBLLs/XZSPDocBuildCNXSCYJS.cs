...
namespace Taizhou.PLE.BLL.XZSPBLLs.XZSPDocBuildBLLs
{
    public partial class XZSPDocBuild
    {
        ...
        /// 生成永久审查意见书
        /// </summary>
        /// <param name="xzspForm">行政审批表单</param>
        /// <param name="docTempName">文书模版名称</param>
        /// <param name="xzspForm"></param>
        public static string XZSPDocBuildCNXSCYJS(XZSPForm xzspForm, string docTempName)
        {
            string tempFilePath, saveWordPath, saveDOCPATH, relativeDOCPATH;

            string docRelativePath = DocToPdf.BuildDocByTemplate(docTempName,
            ...
            dic.Add("$施工开始期限$", strkssgqx);
            dic.Add("$施工结束期限$", strjssgqx);
            dic.Add("$开始设置期限$", strksszqx);
            dic.Add("$结束设置期限$", strjsszqx);
            dic.Add("$审查意见$", form105.SCYJ);
            dic.Add("$承办人$", cbrName);
            dic.Add("$承办机构$", cbjgName);
            dic.Add("$处理时间$", form106.ProcessTime.Value.ToString("yyyy年MM月dd日"));
            ...
            return docRelativePath + ";" + docTempName;
        }
        ...
        /// <param name="docTempName">文书模版名称</param>
        /// <returns></returns>
        public static string XZSPDocBuildLSXSCYJS(XZSPForm xzspForm, string docTempName)
        {
            string tempFilePath, saveWordPath, saveDOCPATH, relativeDOCPATH;
