myFocus.pattern.extend({//*********************games******************
	'mF_games_tb':function(settings,$){
		...
		var $diyPrev=$focus.find('.diyPrev');
		var $diyNext=$focus.find('.diyNext');
		...
		/*$thumbBox.find('img').each(function(){this.style.height=(p.thumbHeight-4)+'px';});*///10px margin+3px border
		...
		$diyPrev.bind('click',function(){$focus.run('-=1')});
		$diyNext.bind('click',function(){$focus.run('+=1')});
